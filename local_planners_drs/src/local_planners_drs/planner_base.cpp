#include <local_planners_drs/planner_base.hpp>
...
void Base::initialize(const Parameters& parameters)
{
  parameters_ = parameters;
}

Base::Output Base::execute()
{
...
// Other steps
void Base::computeDistanceAndOrientationToGoal()
{
...
bool Base::checkFailure()
{
  return false; // TODO implement proper solution
}

// Interfaces for external data
void Base::setImageRgb(const cv::Mat& img, const Pose3& T_b_s)
{
...
void Base::setImageRgbd(const cv::Mat& img, const Pose3& T_b_s)
{
...
void Base::setImageDepth(const cv::Mat& img, const Pose3& T_b_s)
{
...
void Base::setPointCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud, const Pose3& T_b_s)
{
  point_cloud_ = cloud;
  T_b_s_pc_ = T_b_s;
  sensing_ready_ = true;
...
void Base::setGridMap(const grid_map::GridMap& grid_map, const Pose3& T_b_s)
{
  grid_map_ = grid_map;
  T_b_s_gm_ = T_b_s;
  sensing_ready_ = true;
...
// Set single goal
void Base::setGoalInFixed(const Pose3& T_f_g, const Pose3& T_f_b) 
{
  T_f_g_ = T_f_g;
  T_f_b_start_ = T_f_b;
  sensing_ready_ = true;
...
void Base::setRobotState(const Pose3& T_f_b, const Twist& b_v)
{
  T_f_b_ = T_f_b;
  b_v_ = b_v_;
  sensing_ready_ = true;
...
void Base::setFixedToMapTransform(const Pose3& T_m_f)
{
  T_m_f_ = T_m_f;
}
