...
FalcoController::FalcoController(const ControllerParameters& params) : 
  ...
  // load paths
  loadPathsFromFile(controller_params_.config_folder_);

  // voxel filter
  voxel_filter_.setLeafSize(params_.voxel_size_filter_, params_.voxel_size_filter_, params_.voxel_size_filter_);
};

void FalcoController::updateControllerParameters(ros::NodeHandle& node_handle) { 
  // Perceptive controller params
  controller_params_.config_folder_   = ros::package::getPath("locally_reactive_controller") + "/config/controllers/falco/";
  // falco params
  controller_params_.goal_clearance_              = utils::getParameterDefault<double>(node_handle, "goal_clearance", 0.1);
  controller_params_.forward_linear_gain_p_       = utils::getParameterDefault<double>(node_handle, "forward_linear_gain_p", 0.1);
  controller_params_.forward_angular_gain_p_      = utils::getParameterDefault<double>(node_handle, "forward_angular_gain_p", 0.1);
  controller_params_.linear_acceleration_         = utils::getParameterDefault<double>(node_handle, "linear_acceleration", 0.2);
  controller_params_.path_scale_                  = utils::getParameterDefault<double>(node_handle, "path_scale", 1.25);
  controller_params_.path_scale_step_             = utils::getParameterDefault<double>(node_handle, "path_scale_step", 0.25);
  controller_params_.min_path_scale_              = utils::getParameterDefault<double>(node_handle, "min_path_scale", 0.75);
  controller_params_.path_range_step_             = utils::getParameterDefault<double>(node_handle, "path_range_step", 0.5);
  controller_params_.min_path_range_              = utils::getParameterDefault<double>(node_handle, "min_path_range", 1.0);
  controller_params_.look_ahead_distance_         = utils::getParameterDefault<double>(node_handle, "look_ahead_distance", 0.5);
  controller_params_.differential_mode_cost_      = utils::getParameterDefault<double>(node_handle, "differential_mode_cost", 1.0);
  controller_params_.goal_cost_                   = utils::getParameterDefault<double>(node_handle, "goal_cost", 1.0);
  controller_params_.traversability_cost_         = utils::getParameterDefault<double>(node_handle, "traversability_cost", 1.0);
  controller_params_.point_per_path_thr_          = utils::getParameterDefault<int>(node_handle,  "point_per_path_thr", 2);
  controller_params_.check_rotational_collisions_ = utils::getParameterDefault<bool>(node_handle, "check_rotational_collisions", true);
  controller_params_.use_path_crop_by_goal_       = utils::getParameterDefault<bool>(node_handle, "crop_path_by_goal", true);

  path_scale_ = controller_params_.path_scale_;
...
void FalcoController::setupDynamicReconfigureServer(ros::NodeHandle& node_handle)
{
  ...
  dynamic_reconfigure_server_ = std::make_shared<dynamic_reconfigure::Server<locally_reactive_controller::FalcoControllerConfig>>(node_handle);
...
void FalcoController::dynamicReconfigureCallback(locally_reactive_controller::FalcoControllerConfig &config, uint32_t level)
{
  // Falco parameters
  utils::assignAndPrintDiff("goal_clearance", controller_params_.goal_clearance_, config.goal_clearance);
  utils::assignAndPrintDiff("forward_linear_gain_p", controller_params_.forward_linear_gain_p_, config.forward_linear_gain_p);
  utils::assignAndPrintDiff("forward_angular_gain_p", controller_params_.forward_angular_gain_p_, config.forward_angular_gain_p);
  utils::assignAndPrintDiff("linear_acceleration", controller_params_.linear_acceleration_, config.linear_acceleration);
  utils::assignAndPrintDiff("path_scale", controller_params_.path_scale_, config.path_scale);
  utils::assignAndPrintDiff("path_scale_step", controller_params_.path_scale_step_, config.path_scale_step);
  utils::assignAndPrintDiff("min_path_scale", controller_params_.min_path_scale_, config.min_path_scale);
  utils::assignAndPrintDiff("path_range_step", controller_params_.path_range_step_, config.path_range_step);
  utils::assignAndPrintDiff("min_path_range", controller_params_.min_path_range_, config.min_path_range);
  utils::assignAndPrintDiff("look_ahead_distance", controller_params_.look_ahead_distance_, config.look_ahead_distance);
  utils::assignAndPrintDiff("differential_mode_cost", controller_params_.differential_mode_cost_, config.differential_mode_cost);
  utils::assignAndPrintDiff("goal_cost", controller_params_.goal_cost_, config.goal_cost);
  utils::assignAndPrintDiff("traversability_cost", controller_params_.traversability_cost_, config.traversability_cost);
  utils::assignAndPrintDiff("point_per_path_thr", controller_params_.point_per_path_thr_, config.point_per_path_thr);
  utils::assignAndPrintDiff("check_rotational_collisions", controller_params_.check_rotational_collisions_, config.check_rotational_collisions);
  utils::assignAndPrintDiff("use_path_crop_by_goal", controller_params_.use_path_crop_by_goal_, config.use_path_crop_by_goal);
...
ControllerBase::OutputAction FalcoController::computeCommandGoalBehind(Eigen::Vector3d& output_linear_velocity, Eigen::Vector3d& output_angular_velocity){
...
ControllerBase::OutputAction FalcoController::computeCommandTurnToGoal(Eigen::Vector3d& output_linear_velocity, Eigen::Vector3d& output_angular_velocity){
  ...
  // output_linear_velocity(0)  = velocity_direction * cos(error_orientation_to_starting_pose_)* params_.max_forward_linear_velocity_;
  // output_linear_velocity(1)  = velocity_direction * sin(error_orientation_to_starting_pose_)* params_.max_forward_linear_velocity_;
  // output_linear_velocity(0)  = velocity_direction * cos(error_heading_to_carrot)* params_.max_forward_linear_velocity_;
  // output_linear_velocity(1)  = velocity_direction * sin(error_heading_to_carrot)* params_.max_forward_linear_velocity_;

  // compute angular velocity only command
  output_angular_velocity(0) = 0.0;
  output_angular_velocity(1) = 0.0;
  output_angular_velocity(2) = error_heading_to_goal_ * params_.angular_gain_p_;
  // output_angular_velocity(2) = error_heading_to_carrot * params_.angular_gain_p_;
...
ControllerBase::OutputAction FalcoController::computeCommandForward(Eigen::Vector3d& output_linear_velocity, Eigen::Vector3d& output_angular_velocity){

  double error_heading_to_carrot = atan2(intermediate_carrot_(1), intermediate_carrot_(0));
  double error_distance_to_carrot = hypot(intermediate_carrot_(1), intermediate_carrot_(0));
  double velocity_direction = 1.0;

  ROS_INFO_STREAM_THROTTLE(1, "x to carrot: " << intermediate_carrot_(0) << ", y to carrot: " << intermediate_carrot_(1)<< ", distance to carrot: " << error_distance_to_carrot);
  ...
  // // Update velocity
  // if(error_distance_to_carrot >= controller_params_.look_ahead_distance_){
  //   last_linear_velocity_+=controller_params_.linear_acceleration_;
  // } else{
  //   last_linear_velocity_-=controller_params_.linear_acceleration_;
  // }

  // // saturate by max velocity
  // last_linear_velocity_ = std::min(last_linear_velocity_, params_.max_forward_linear_velocity_);

  
  // output_linear_velocity(0)  = velocity_direction * params_.max_forward_linear_velocity_;
  output_linear_velocity(0)  = velocity_direction * params_.max_forward_linear_velocity_ * error_distance_to_goal_;
  // output_linear_velocity(0)  = velocity_direction * last_linear_velocity_;

  if(error_distance_to_carrot >= controller_params_.look_ahead_distance_)
    output_angular_velocity(2) = error_heading_to_carrot * controller_params_.forward_angular_gain_p_;
  else
    output_angular_velocity(2) = error_heading_to_goal_ * controller_params_.forward_angular_gain_p_;

  return ControllerBase::OutputAction::SEND_COMMAND;
}

ControllerBase::OutputAction FalcoController::computeCommandTurnToDestination(Eigen::Vector3d& output_linear_velocity, Eigen::Vector3d& output_angular_velocity){
  double velocity_direction = 1.0;
  ...
  output_linear_velocity(0)  = velocity_direction * cos(error_heading_to_goal_)* params_.max_forward_linear_velocity_;
  output_linear_velocity(1)  = velocity_direction * sin(error_heading_to_goal_)* params_.max_forward_linear_velocity_;
  

  output_angular_velocity(2) = -error_orientation_to_goal_ * params_.angular_gain_p_;
...
ControllerBase::OutputAction FalcoController::computeCommandFinished(Eigen::Vector3d& output_linear_velocity, Eigen::Vector3d& output_angular_velocity){
...
//-------------------------------------------------------------------------------------------------
void FalcoController::computeFreePath() {
  // Initialize variables
  can_rotate_ = true;
  is_colliding_ = false;

  // Path range
  path_range_ = params_.sensor_range_;
  path_range_ = std::max(path_range_, controller_params_.min_path_range_);
  // path_range_ = std::max(path_range_, params_.robot_diameter_);

  // Path scale
  // Store the path scale temporarily
  path_scale_ = controller_params_.path_scale_;
  path_scale_ = std::max(path_scale_, controller_params_.min_path_scale_);

  // Main iteration to check the paths
  while (path_scale_ >= controller_params_.min_path_scale_ && path_range_ >= controller_params_.min_path_range_) {
    // ROS_INFO_STREAM("path_scale: " << path_scale_ << ", path_range_: " << path_range_);
    clearPathsLists();
    computeMapCollisions();
    computePathScores();

    feasible_path_found_ = findBestPath();
    // ROS_INFO_STREAM("[Falco] found best path: " << feasible_path_found_);
...
void FalcoController::clearPathsLists() {
...
void FalcoController::computeMapCollisions() {
  // ROS_INFO("[Falco] compute map collisions");
  // min_angle_to_obstacle_cw_  = utils::deg2rad(-180.0);
  // min_angle_to_obstacle_ccw_ = utils::deg2rad(180.0);
  
  // Precompute some stuff
  // We compute the heading to goal here again since we'll be comparing the paths defined in a 
  // canonical reference frame (base frame) without angular adjustments (such as, front and back symmetries)
  const float heading_to_goal = atan2(dT_base_goal_.translation().y(), dT_base_goal_.translation().x());

  // Iterate over all the map points to find collisions
  int collision_map_size = collision_map_in_base_->points.size();
  for (int i = 0; i < collision_map_size; i++) {

    // Precompute all the properties about the voxel that are reused later
    const float x = (collision_map_in_base_->points[i].x - params_.robot_clearance_) / path_scale_; // normalized by path_scale
    const float y = (collision_map_in_base_->points[i].y - params_.robot_clearance_) / path_scale_; // normalized by path_scale

    const float height = collision_map_in_base_->points[i].z;
    const float normal_x = collision_map_in_base_->points[i].normal_x;
    const float normal_y = collision_map_in_base_->points[i].normal_y;
    const float normal_z = collision_map_in_base_->points[i].normal_z;

    // Get traversability
    // Defined in the intensity channel
    const float traversability = collision_map_in_base_->points[i].intensity;

    // Check the score
    bool is_traversable = traversability > params_.traversable_thr_;

    // Compute distance and angle to obstacle
    float distance_to_obstacle = sqrt(x * x + y * y);
    float angle_to_obstacle = atan2(y, x);

    // If the obstacle is within the path ranges, check collisions
    if (distance_to_obstacle < path_range_ / path_scale_ 
        && (distance_to_obstacle <= ((error_distance_to_goal_ + controller_params_.goal_clearance_) / path_scale_) 
            || !controller_params_.use_path_crop_by_goal_)
        ) {
      ...
      for (int rot_dir = 0; rot_dir < 36; rot_dir++) {
        ...
        float scale_y = (x2 / GRID_VOXEL_OFFSET_X) + (SEARCH_RADIUS / GRID_VOXEL_OFFSET_Y) 
    ...
    if(controller_params_.check_rotational_collisions_ && !is_traversable){
      if ((distance_to_obstacle < params_.robot_diameter_ / path_scale_) && 
          ( (fabs(x) < params_.robot_length_ / path_scale_ / 2.0) || (fabs(y) < params_.robot_width_ / path_scale_ / 2.0))) {
        
        // ROS_WARN("Potential collision");
...
void FalcoController::computePathScores() {
  // ROS_INFO("[Falco] compute_path_scores");
  float heading_to_goal = atan2(dT_base_goal_.translation().y(), dT_base_goal_.translation().x());
  ...
  for (int i = 0; i < 36 * NUM_PATHS; i++) {
    ...
    if (clear_path_list_[i] <= controller_params_.point_per_path_thr_) {
      ...
      float goal_probability = utils::vonMises(end_dir_path_list_[i % NUM_PATHS] + rot_ang, heading_to_goal, controller_params_.goal_cost_);

      // Traversability probability
      // Average probability over the traversability of all the voxels crossed by the path
      float traversability_probability = exp(controller_params_.traversability_cost_* path_traversability_score_[i] / path_traversability_crossed_voxels_[i]);
      ...
      if(params_.differential_drive_){
        differential_drive_probability = 0.5*(utils::vonMises(rot_ang, 0, controller_params_.differential_mode_cost_) + utils::vonMises(rot_ang, M_PI, controller_params_.differential_mode_cost_));
      }
...
bool FalcoController::findBestPath() {
  // ROS_INFO("[Falco] find best path");
  // Select the group with the maximum score
  // This is a simple linear search
  ROS_WARN_STREAM_THROTTLE(1, "Previous path: " << (best_path_group_id_ % NUM_GROUPS) << ", score: " << best_path_group_score_);
  ...
  if (best_path_group_id_ >= 0) {
    ROS_WARN_STREAM_THROTTLE(1, "Selected path: " << (best_path_group_id_ % NUM_GROUPS) << ", score: " << best_path_group_score_);
    ...
    for (int i = 0; i < selected_path_length; i++) {
      ...
      if (dis <= path_range_ / path_scale_ && dis <= error_distance_to_goal_ / path_scale_) {
        best_path_[i] = Eigen::Vector3d(path_scale_ * (cos(rot_ang) * x - sin(rot_ang) * y),
                                        path_scale_ * (sin(rot_ang) * x + cos(rot_ang) * y),
                                        path_scale_ * z);
      } else {
        best_path_.resize(i);
        break;
    ...
    for (int i = 0; i < 36 * NUM_PATHS; i++) {
      ...
      if (clear_path_list_[i] < controller_params_.point_per_path_thr_) {
        ...
        for (int j = 0; j < free_path_length; j++) {
          ...
          point.intensity = path_group_likelihood_[NUM_GROUPS * rot_dir + path_list_[i % NUM_PATHS]]; //path_likelihood_[i]; //clear_path_list_[i]; //path_likelihood_[i];
          // point.intensity = path_likelihood_[i];

          float dis = sqrt(x * x + y * y);
          if (dis <= path_range_ / path_scale_ && (dis <= (error_distance_to_goal_ + controller_params_.goal_clearance_) / path_scale_ || !controller_params_.use_path_crop_by_goal_)) {
  ...
  if (best_path_group_id_ < 0) {
    if (path_scale_ >= controller_params_.min_path_scale_ + controller_params_.path_scale_step_) {
      path_scale_ -= controller_params_.path_scale_step_;
      path_range_ = params_.sensor_range_ * path_scale_ / controller_params_.path_scale_;
    } else {
      path_range_ -= controller_params_.path_range_step_;
    }
  } else {
    return true;
  }
...
//-------------------------------------------------------------------------------------------------
void FalcoController::computeIntermediateCarrot() {
  if(best_path_.empty()){
    ROS_ERROR_THROTTLE(1, "Best path is empty");
    return;
  }

  // Search carrot in path that is farther than look_ahead_distance
  intermediate_carrot_ = Eigen::Vector3d::Zero();
  for(auto point : best_path_){
    double distance = point.norm();
    if(distance > controller_params_.look_ahead_distance_){
...
//   // convert carrot to fixed frame
//  intermediate_carrot = f_T_fb_t_ * intermediate_carrot_;
}

//-------------------------------------------------------------------------------------------------
// Load files
//-------------------------------------------------------------------------------------------------
void FalcoController::loadPathsFromFile(std::string folder){
  ...
  ROS_INFO_STREAM("Loading files from folder " << folder << "");
...
FILE* FalcoController::openFile(std::string& file){
  ROS_INFO_STREAM("Opening file " << file);

  FILE *file_ptr = fopen(file.c_str(), "r");
  if (file_ptr == NULL) {
    ROS_ERROR("Error: Cannot read input files, exit.");
    exit(1);
  }
...
int FalcoController::readPlyHeader(FILE* file_ptr){
  char str[50];
  int val, point_num;
  ...
  while (str_cur != "end_header") {
    ...
    if (val != 1) {
      ROS_ERROR("Error reading input files, exit.");
      exit(1);
    }
...
void FalcoController::loadPrecomputedPaths(std::string folder) {
...
void FalcoController::loadPathsList(std::string folder) {
  std::string file_name = folder + "path_list.ply";

  FILE *file_ptr = openFile(file_name);
  if (NUM_PATHS != readPlyHeader(file_ptr)) {
    ROS_ERROR("Error: Incorrect path number, exit.");
    exit(1);
  }
...
void FalcoController::loadPaths(std::string folder)
{
...
void FalcoController::loadCorrespondences(std::string folder) {
...
void FalcoController::convertElevationMapToCloud() {
  ...
  grid_map::Length length(2.0*params_.sensor_range_, 2.0*params_.sensor_range_);
  ...
  // ROS_INFO_STREAM("Terrain map has " << collision_map_->points.size() << " points");
}
...
void FalcoController::updateElevationMapFrame() {
  ...
  tf_listener_.waitForTransform(params_.base_frame_, grid_map_.getFrameId(), ros::Time(0), ros::Duration(2.0));
  try {
    tf_listener_.lookupTransform(params_.base_frame_, grid_map_.getFrameId(), ros::Time(0), map_to_base_transform);
...
{
  free_paths_pub_         = node_handle.advertise<sensor_msgs::PointCloud2>("/locally_reactive_controller/falco/free_paths", 10);
  collision_map_pub_      = node_handle.advertise<sensor_msgs::PointCloud2>("/locally_reactive_controller/falco/collision_map", 10);
}
...
void FalcoController::publishVisualizations(const std_msgs::Header& header)
{
  ...
  collision_map_cloud.header.frame_id = params_.base_frame_;
  ...
  free_paths_cloud.header.frame_id = params_.base_frame_;
