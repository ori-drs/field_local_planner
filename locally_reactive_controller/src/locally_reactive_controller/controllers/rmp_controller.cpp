...
//-------------------------------------------------------------------------------------------------
RmpController::RmpController(const ControllerParameters& params) : 
...
void RmpController::updateControllerParameters(ros::NodeHandle& node_handle) {
  controller_params_.config_folder_   = ros::package::getPath("locally_reactive_controller") + "/config/controllers/rmp/";

  // RMP params
  controller_params_.sphere_radius_factor_      = utils::getParameterDefault<double>(node_handle, "sphere_radius_factor", 1.0);
  controller_params_.geodesic_goal_gain_        = utils::getParameterDefault<double>(node_handle, "geodesic_goal_gain", 7.0);
  controller_params_.geodesic_goal_offset_      = utils::getParameterDefault<double>(node_handle, "geodesic_goal_offset", 0.5);
  controller_params_.geodesic_goal_steepness_   = utils::getParameterDefault<double>(node_handle, "geodesic_goal_steepness", 1.0);
  controller_params_.euclidean_goal_gain_       = utils::getParameterDefault<double>(node_handle, "euclidean_goal_gain", 7.0);
  controller_params_.euclidean_goal_offset_     = utils::getParameterDefault<double>(node_handle, "euclidean_goal_offset", 0.5);
  controller_params_.euclidean_goal_steepness_  = utils::getParameterDefault<double>(node_handle, "euclidean_goal_steepness", 1.0);
  controller_params_.damping_                   = utils::getParameterDefault<double>(node_handle, "damping", 1.0);
  controller_params_.obstacle_gain_             = utils::getParameterDefault<double>(node_handle, "obstacle_gain", 1.0);
  controller_params_.obstacle_offset_           = utils::getParameterDefault<double>(node_handle, "obstacle_offset", 0.1);
  controller_params_.obstacle_steepness_        = utils::getParameterDefault<double>(node_handle, "obstacle_steepness", 1.0);
  controller_params_.heading_gain_              = utils::getParameterDefault<double>(node_handle, "heading_gain", 1.0);
  controller_params_.heading_offset_            = utils::getParameterDefault<double>(node_handle, "heading_offset", 0.5);
  controller_params_.heading_steepness_         = utils::getParameterDefault<double>(node_handle, "heading_steepness", 1.0);
  controller_params_.regularization_            = utils::getParameterDefault<double>(node_handle, "acceleration_regularization", 0.1);
  controller_params_.integration_time_          = utils::getParameterDefault<double>(node_handle, "integration_time", 0.3);

  controller_params_.geodesic_goal_weight_         = utils::getParameterDefault<double>(node_handle, "geodesic_goal_weight", 1.0);
  controller_params_.euclidean_goal_weight_ = utils::getParameterDefault<double>(node_handle, "euclidean_goal_weight", 1.0);
  controller_params_.damping_weight_      = utils::getParameterDefault<double>(node_handle, "damping_weight", 1.0);
  controller_params_.obstacle_weight_     = utils::getParameterDefault<double>(node_handle, "obstacle_weight", 1.0);
  controller_params_.heading_weight_      = utils::getParameterDefault<double>(node_handle, "heading_weight", 1.0);
  controller_params_.check_internal_collisions_ = utils::getParameterDefault<bool>(node_handle, "check_internal_collisions", false); 
...
void RmpController::setupDynamicReconfigureServer(ros::NodeHandle& node_handle)
{
  ...
  dynamic_reconfigure_server_ = std::make_shared<dynamic_reconfigure::Server<locally_reactive_controller::RmpControllerConfig>>(node_handle);
  
  dynamic_reconfigure_callback_ = boost::bind(&RmpController::dynamicReconfigureCallback, this, _1, _2);
  dynamic_reconfigure_server_->setCallback(dynamic_reconfigure_callback_);
}

void RmpController::dynamicReconfigureCallback(locally_reactive_controller::RmpControllerConfig &config, uint32_t level)
{
  // RMP parameters
  utils::assignAndPrintDiff("sphere_radius_factor", controller_params_.sphere_radius_factor_, config.sphere_radius_factor);
  utils::assignAndPrintDiff("goal_gain", controller_params_.geodesic_goal_gain_, config.geodesic_goal_gain);
  utils::assignAndPrintDiff("goal_offset", controller_params_.geodesic_goal_offset_, config.geodesic_goal_offset);
  utils::assignAndPrintDiff("goal_steepness", controller_params_.geodesic_goal_steepness_, config.geodesic_goal_steepness);
  utils::assignAndPrintDiff("euclidean_goal_gain", controller_params_.euclidean_goal_gain_, config.euclidean_goal_gain);
  utils::assignAndPrintDiff("euclidean_goal_offset", controller_params_.euclidean_goal_offset_, config.euclidean_goal_offset);
  utils::assignAndPrintDiff("euclidean_goal_steepness", controller_params_.euclidean_goal_steepness_, config.euclidean_goal_steepness);
  utils::assignAndPrintDiff("damping", controller_params_.damping_, config.damping);
  utils::assignAndPrintDiff("obstacle_gain", controller_params_.obstacle_gain_, config.obstacle_gain);
  utils::assignAndPrintDiff("obstacle_offset", controller_params_.obstacle_offset_, config.obstacle_offset);
  utils::assignAndPrintDiff("obstacle_steepness", controller_params_.obstacle_steepness_, config.obstacle_steepness);
  utils::assignAndPrintDiff("heading_gain", controller_params_.heading_gain_, config.heading_gain);
  utils::assignAndPrintDiff("heading_offset", controller_params_.heading_offset_, config.heading_offset);
  utils::assignAndPrintDiff("heading_steepness", controller_params_.heading_steepness_, config.heading_steepness);
  utils::assignAndPrintDiff("acceleration_regularization", controller_params_.regularization_, config.acceleration_regularization);
  utils::assignAndPrintDiff("integration_time", controller_params_.integration_time_, config.integration_time);

  utils::assignAndPrintDiff("geodesic_goal_weight", controller_params_.geodesic_goal_weight_, config.geodesic_goal_weight);
  utils::assignAndPrintDiff("euclidean_goal_weight", controller_params_.euclidean_goal_weight_, config.euclidean_goal_weight);
  utils::assignAndPrintDiff("damping_weight", controller_params_.damping_weight_, config.damping_weight);
  utils::assignAndPrintDiff("obstacle_weight", controller_params_.obstacle_weight_, config.obstacle_weight);
  utils::assignAndPrintDiff("heading_weight", controller_params_.heading_weight_, config.heading_weight);
  utils::assignAndPrintDiff("check_internal_collisions", controller_params_.check_internal_collisions_, config.check_internal_collisions);
...
void RmpController::prepareControlPoints() {
  ...
  YAML::Node cp_config = YAML::LoadFile(controller_params_.config_folder_ + "control_points.yaml");
  ...
  for(YAML::const_iterator it=cp_config["control_points"].begin();it != cp_config["control_points"].end();++it) {
    ...
    control_point.point_ = flip_front_direction * Eigen::Vector2d(robot_half_length * factors[0], robot_half_width * factors[1]);
    ...
    control_point.affected_by_obstacle_ = utils::get<bool>(cp_attributes, "affected_by_obstacle", true);
    // Radius
    control_point.radius_ = std::min(params_.robot_length_, params_.robot_width_) * 0.5 * controller_params_.sphere_radius_factor_;//utils::get<float>(cp_attributes, "radius", 0.1);
...
void RmpController::customPreProcessController() {
  ...
  try {
    // Listen transform
    tf_listener_.lookupTransform(map_frame, params_.fixed_frame_, ros::Time(0), fixed_to_map_transform);
  ...
  // Create gtsam poses
  gtsam_current_pose_in_fixed_ = gtsam::Pose2(f_T_fb_t_2d_.translation().x(),
                                              f_T_fb_t_2d_.translation().y(),
  ...
  gtsam_current_pose_in_map_ = gtsam::Pose2(current_pose_in_map_.translation().x(),
                                            current_pose_in_map_.translation().y(),
                                            pose_in_map_angle);
  
  Eigen::AngleAxisd axis_angle(f_Tgoal_fb_t_.rotation());
  double goal_angle = axis_angle.axis()(2) * axis_angle.angle();
  gtsam_current_goal_in_fixed_ = gtsam::Pose2(f_Tgoal_fb_t_.translation().x(),
                                              f_Tgoal_fb_t_.translation().y(),
                                              goal_angle);
...
ControllerBase::OutputAction RmpController::computeCommandGoalBehind(Eigen::Vector3d& output_linear_velocity, 
...
ControllerBase::OutputAction RmpController::computeCommandTurnToGoal(Eigen::Vector3d& output_linear_velocity,
                                                                     Eigen::Vector3d& output_angular_velocity){
  return computeCommandForward(output_linear_velocity, output_angular_velocity);
}

ControllerBase::OutputAction RmpController::computeCommandForward(Eigen::Vector3d& output_linear_velocity,
                                                                  Eigen::Vector3d& output_angular_velocity){
...
ControllerBase::OutputAction RmpController::computeCommandTurnToDestination(Eigen::Vector3d& output_linear_velocity, 
                                                                            Eigen::Vector3d& output_angular_velocity){
  return computeCommandForward(output_linear_velocity, output_angular_velocity);
}

ControllerBase::OutputAction RmpController::computeCommandFinished(Eigen::Vector3d& output_linear_velocity,
                                                                   Eigen::Vector3d& output_angular_velocity){
...
void RmpController::computeOptimalAcceleration() {
  ...
  for(size_t i=0; i<control_points_.size(); i++) {
    ...
    // Obstacles
    {
      ...
      Vector2 gradient_in_base_frame = gtsam_current_pose_in_map_.rotation().inverse() * Vector2(obstacle_grad_x, obstacle_grad_y);

      // Create Obstacle RMP
      SdfObstacle2Rmp obstacle_rmp(cp.point(),
                                  cp.apply(velocity_2d_),
                                  obstacle_distance,
      ...
      if(control_points_.at(i).affected_by_obstacle_) {
        if(params_.differential_drive_) {
          problem.addRmp(applyControlPoint2(cp_, applyDifferentialModel(motion_model_, acc_diff)), obstacle_rmp);
        } else {
  ...
  // Damping RMP
  DampingRmp damping_rmp(gtsam_current_pose_in_fixed_,
                            velocity_2d_,
                            controller_params_.damping_,
  ...
  // Goal Heading RMP
  EuclideanGoalRmp goal_with_heading_rmp(gtsam_current_pose_in_fixed_, 
                                velocity_2d_,
  ...
  // Heading RMP
  HeadingRmp heading_rmp(gtsam_current_pose_in_fixed_, 
                         velocity_2d_,
                         Vector2(1.0, 0.0),
                         goal_distance,
  ...
  if(params_.differential_drive_) {
    ...
    problem.addRmp(acc_diff, Regularization2Rmp(optimal_acc_diff, controller_params_.regularization_));
    ...
    // Regularization RMP
    problem.addRmp(acc_se2, Regularization3Rmp(optimal_acc_, controller_params_.regularization_));
...
void RmpController::setupVisualizations(ros::NodeHandle& node_handle) {
  control_points_pub_ = node_handle.advertise<visualization_msgs::MarkerArray>("/locally_reactive_controller/rmp/control_points", 10);
  accelerations_pub_  = node_handle.advertise<visualization_msgs::MarkerArray>("/locally_reactive_controller/rmp/accelerations", 10);
}

void RmpController::publishVisualizations(const std_msgs::Header& header) {
  // Publish Control points (base)
  visualization_msgs::MarkerArray control_points_array = controlPointsToMarkerArray(control_points_, header);
  control_points_pub_.publish(control_points_array);

  // // Publish accelerations
  visualization_msgs::MarkerArray accs_array = accelerationsToMarkerArray(vis_accelerations_, header);
  accelerations_pub_.publish(accs_array);
}

visualization_msgs::MarkerArray RmpController::controlPointsToMarkerArray(const std::vector<ControlPointStruct>& control_points,
...
visualization_msgs::MarkerArray RmpController::accelerationsToMarkerArray(const std::vector<AccelerationVisualization>& accelerations,
                                                             const std_msgs::Header& header) {
  int i = 0;
