...
template<typename T>
T getParameterDefault(const ros::NodeHandle& node_handle, const std::string &param, const T &defaultValue) {
...
/// @author David Wisth
template <typename T> T get(const YAML::Node &node, const int &param, const T &defaultValue, bool silent=false) {
  if (!node[param]) {
    //if(!silent) std::cout << std::left << std::setw(COL_WIDTH) << param << ": Not found. Using default value: " << defaultValue << std::endl;
    return defaultValue;
  }
...
template <typename T> T get(const YAML::Node &node, const std::string &param, const T &defaultValue, bool silent=false) {
  if (!node[param]) {
    //if(!silent) std::cout << std::left << std::setw(COL_WIDTH) << param << ": Not found. Using default value: " << defaultValue << std::endl;
    return defaultValue;
  }
...
static inline void clipValue(double &value, double max_value){
  if (value > max_value)
    value = max_value;
