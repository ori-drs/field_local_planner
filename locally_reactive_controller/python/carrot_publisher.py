...
path_pkg = rospack.get_path('locally_reactive_controller')

carrot_file = path_pkg + '/config/meshes/carrot/carrot.dae'
carrot_marker_pub = rospy.Publisher('/locally_reactive_controller/real_carrot', Marker, queue_size=10)
...
if __name__ == '__main__':
    rospy.init_node('locally_reactive_controller_carrot_publisher')

    # Read parameter with mesh
    # world_option = rospy.get_param('~world', 'arena_simple')
   
    # Objects
    goal_topic = rospy.get_param('~goal_topic', '/locally_reactive_controller/current_goal')
    rospy.Subscriber(goal_topic, PoseStamped, goal_callback, queue_size=10)

    controller_status_topic = rospy.get_param('~controller_status_topic', '/locally_reactive_controller/controller_status')
