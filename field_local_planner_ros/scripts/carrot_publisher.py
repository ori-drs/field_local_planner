...
path_pkg = rospack.get_path('field_local_planner')

carrot_file = path_pkg + '/config/meshes/carrot/carrot.dae'
carrot_marker_pub = rospy.Publisher('/field_local_planner/real_carrot', Marker, queue_size=10)
...
if __name__ == '__main__':
    ...
    # Objects
    goal_topic = rospy.get_param('~goal_topic', '/field_local_planner/current_goal')
    rospy.Subscriber(goal_topic, PoseStamped, goal_callback, queue_size=10)

    controller_status_topic = rospy.get_param('~controller_status_topic', '/field_local_planner/controller_status')
