...
LocalPlannerHandler::LocalPlannerHandler(ros::NodeHandle& nh) : action_server_(nh, "action_server", false) {
...
void LocalPlannerHandler::loadPlugin(ros::NodeHandle& nh) {
  ...
  try {
    // Create instance
    local_planner_ = plugin_loader_->createInstance(planner_plugin.str());
...
void LocalPlannerHandler::setupRos(ros::NodeHandle& nh) {
  ...
  //   rgb_image_sub_ = nh.subscribe(std::string(rgb_image_topic), 1, &LocalPlannerHandler::imageRgbCallback, this);
  // }

  // std::string rgbd_image_topic = utils::getParameterDefault(nh, "rgbd_image_topic", std::string(""));
  // if (!rgbd_image_topic.empty()) {
  //   // TODO add synchronized subscriber
  //   rgbd_image_sub_ = nh.subscribe(std::string(rgbd_image_topic), 1, &LocalPlannerHandler::imageRgbdCallback, this);
  // }

  // std::string depth_image_topic = utils::getParameterDefault(nh, "depth_image_topic", std::string(""));
  // if (!depth_image_topic.empty()) {
  //   // TODO add synchronized subscriber
  //   depth_image_sub_ = nh.subscribe(std::string(depth_image_topic), 1, &LocalPlannerHandler::imageDepthCallback, this);
  // }

  std::string point_cloud_topic = utils::getParameterDefault(nh, "point_cloud_topic", std::string(""));
  if (!point_cloud_topic.empty()) {
    point_cloud_sub_ = nh.subscribe(std::string(point_cloud_topic), 1, &LocalPlannerHandler::pointCloudCallback, this);
  }
...
void LocalPlannerHandler::poseCallback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& pose_msg) {
  local_planner_->setPose(pose_msg->pose.pose, pose_msg->header);
}

void LocalPlannerHandler::twistCallback(const geometry_msgs::TwistWithCovarianceStampedConstPtr& twist_msg) {
  local_planner_->setVelocity(twist_msg->twist.twist, twist_msg->header);
}

void LocalPlannerHandler::odometryCallback(const nav_msgs::OdometryConstPtr& odo_msg) {
  local_planner_->setPose(odo_msg->pose.pose, odo_msg->header);
  local_planner_->setVelocity(odo_msg->twist.twist, odo_msg->header);
}

// // Sensor callbacks
// void LocalPlannerHandler::imageRgbCallback(const sensor_msgs::ImageConstPtr& rgb_msg, const sensor_msgs::CameraInfoConstPtr& info_msg) {
//   local_planner_->setImageRgb(rgb_msg, info_msg);
// }
...
//   local_planner_->setImageRgbd(rgb_msg, depth_msg, info_msg);
// }

// void LocalPlannerHandler::imageDepthCallback(const sensor_msgs::ImageConstPtr& depth_msg, const sensor_msgs::CameraInfoConstPtr&
// info_msg) {
//   local_planner_->setImageDepth(depth_msg, info_msg);
// }

void LocalPlannerHandler::pointCloudCallback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
  local_planner_->setPointCloud(cloud_msg);
}

void LocalPlannerHandler::gridMapCallback(const grid_map_msgs::GridMap& gridmap_msg) {
  local_planner_->setGridMap(gridmap_msg);
}

// Goal callback
void LocalPlannerHandler::goalCallback(const geometry_msgs::PoseStampedConstPtr& goal_msg) {
  local_planner_->setGoal(goal_msg->pose, goal_msg->header);
}

// Joystick callback
void LocalPlannerHandler::joyTwistCallback(const geometry_msgs::TwistConstPtr& twist_msg) {
  local_planner_->setJoyCommand(*twist_msg);
}
...
void LocalPlannerHandler::publishTwist(const geometry_msgs::Twist& twist) {
  if (output_twist_type_ == "twist") {
    ...
    twist_stamped.header.frame_id = local_planner_->getBaseFrame();
